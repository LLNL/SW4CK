...
void curvilinear4sg_ci(
    ...
    float_sw4* __restrict__ a_met1,float_sw4* __restrict__ a_met2,
    float_sw4* __restrict__ a_met3,float_sw4* __restrict__ a_met4,
  ...
  for (int i = 1; i < 2; i++) {  // 0 has the smaller datatset
    ...
    for (int p = 0; p < 1; p++)
      curvilinear4sg_ci(optr[6], optr[7], optr[8], optr[9], optr[10], optr[11],
			...
			mua_ptr+base, lambdaa_ptr+base, 
			met_ptr+base4+nijk,met_ptr+base4+2*nijk,
			met_ptr+base4+3*nijk,met_ptr+base4+4*nijk,
			jac_ptr+base,
